...
AITunerDialog::AITunerDialog(wxWindow* parent)
    : DPIDialog(parent, wxID_ANY, _L("AI Settings Tuner"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    build();
    load_ai_settings();
    update_provider_ui();
    ...
    if (!key.empty()) {
        ...
        // We defer prompt to Show call? Or just do it now if possible.
        // Doing it in constructor is risky if parent isn't ready, but CallAfter works.
        this->CallAfter([this]() { prompt_for_api_key(); });
    }
}

AITunerDialog::~AITunerDialog() {}

void AITunerDialog::build()
{
    ...
    auto intro_text =
        new wxStaticText(this, wxID_ANY,
                         _L("Describe the defects you are seeing (optional; e.g., 'strings between posts', 'warping at corners').\n"
                            "The AI will analyze all current print settings (including advanced options) and suggest optimizations."));
    ...
    m_provider_choice->Bind(wxEVT_CHOICE, &AITunerDialog::on_provider_changed, this);
    ...
    auto objectives_box  = new wxStaticBoxSizer(wxVERTICAL, this, _L("Optimization Objectives"));
    auto objectives_grid = new wxFlexGridSizer(3, 3, 6, 8);
    objectives_grid->Add(new wxStaticText(this, wxID_ANY, _L("Quality")), 0, wxALIGN_CENTER_VERTICAL);
    m_quality_slider = new wxSlider(this, wxID_ANY, 50, 0, 100, wxDefaultPosition, wxDefaultSize, wxSL_HORIZONTAL);
    m_quality_slider->Bind(wxEVT_SLIDER, &AITunerDialog::on_objective_changed, this);
    objectives_grid->Add(m_quality_slider, 1, wxEXPAND);
    m_quality_value = new wxStaticText(this, wxID_ANY, "50");
    objectives_grid->Add(m_quality_value, 0, wxALIGN_CENTER_VERTICAL);

    objectives_grid->Add(new wxStaticText(this, wxID_ANY, _L("Speed")), 0, wxALIGN_CENTER_VERTICAL);
    m_speed_slider = new wxSlider(this, wxID_ANY, 50, 0, 100, wxDefaultPosition, wxDefaultSize, wxSL_HORIZONTAL);
    m_speed_slider->Bind(wxEVT_SLIDER, &AITunerDialog::on_objective_changed, this);
    objectives_grid->Add(m_speed_slider, 1, wxEXPAND);
    m_speed_value = new wxStaticText(this, wxID_ANY, "50");
    objectives_grid->Add(m_speed_value, 0, wxALIGN_CENTER_VERTICAL);

    objectives_grid->Add(new wxStaticText(this, wxID_ANY, _L("Strength")), 0, wxALIGN_CENTER_VERTICAL);
    m_strength_slider = new wxSlider(this, wxID_ANY, 50, 0, 100, wxDefaultPosition, wxDefaultSize, wxSL_HORIZONTAL);
    m_strength_slider->Bind(wxEVT_SLIDER, &AITunerDialog::on_objective_changed, this);
    objectives_grid->Add(m_strength_slider, 1, wxEXPAND);
    m_strength_value = new wxStaticText(this, wxID_ANY, "50");
    objectives_grid->Add(m_strength_value, 0, wxALIGN_CENTER_VERTICAL);

    objectives_grid->AddGrowableCol(1, 1);
    objectives_box->Add(objectives_grid, 1, wxEXPAND | wxALL, 8);
    main_sizer->Add(objectives_box, 0, wxLEFT | wxRIGHT | wxBOTTOM | wxEXPAND, 10);

    // Input Area
    m_input_desc = new wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxSize(400, 100), wxTE_MULTILINE);
    main_sizer->Add(m_input_desc, 1, wxALL | wxEXPAND, 10);

    // Controls
    auto btn_sizer = new wxBoxSizer(wxHORIZONTAL);
    m_btn_analyze  = new wxButton(this, wxID_ANY, _L("Analyze & Tune"));
    m_btn_analyze->Bind(wxEVT_BUTTON, &AITunerDialog::on_analyze, this);

    m_btn_key = new wxButton(this, wxID_ANY, _L("API Key..."));
    m_btn_key->Bind(wxEVT_BUTTON, &AITunerDialog::on_change_key, this);
    ...
    // Results Area
    auto result_label = new wxStaticText(this, wxID_ANY, _L("AI Suggestions:"));
    main_sizer->Add(result_label, 0, wxLEFT | wxTOP, 10);

    m_result_view = new wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxSize(400, 200), wxTE_MULTILINE | wxTE_READONLY);
    ...
    m_diff_view = new wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxSize(400, 160), wxTE_MULTILINE | wxTE_READONLY);
    main_sizer->Add(m_diff_view, 1, wxALL | wxEXPAND, 10);

    // Bottom Buttons
    auto bottom_sizer = new wxBoxSizer(wxHORIZONTAL);
    m_btn_apply       = new wxButton(this, wxID_ANY, _L("Apply Changes"));
    m_btn_apply->Bind(wxEVT_BUTTON, &AITunerDialog::on_apply, this);
    m_btn_apply->Disable();
    ...
    SetMinSize(wxSize(520, 720));
    Fit();
    CenterOnParent();
}

void AITunerDialog::on_analyze(wxCommandEvent& event)
{
    std::string user_input = m_input_desc->GetValue().ToStdString();
    if (user_input.empty()) {
        user_input = "No issue description provided. Optimize the full print profile for balanced quality, reliability, and speed.";
    }

    PresetBundle* preset_bundle = wxGetApp().preset_bundle;
    if (!preset_bundle) {
        wxMessageBox(_L("Preset settings are not available yet. Please open a project or restart OrcaSlicer."), _L("AI Settings Tuner"),
                     wxICON_ERROR | wxOK);
        return;
    }
    ...
    if (provider == "gemini") {
        ...
        if (key.empty()) {
            wxMessageBox(_L("Gemini API key is missing. Please set it first."), _L("AI Settings Tuner"), wxICON_WARNING | wxOK);
            prompt_for_api_key();
            return;
        }
        m_gemini_client.set_api_key(key);
        ...
        if (endpoint.empty() || model.empty()) {
            wxMessageBox(_L("Local endpoint and model are required for local AI."), _L("AI Settings Tuner"), wxICON_WARNING | wxOK);
            return;
        }
    ...
    if (m_quality_slider)
        objectives.quality = m_quality_slider->GetValue();
    if (m_speed_slider)
        objectives.speed = m_speed_slider->GetValue();
    if (m_strength_slider)
        objectives.strength = m_strength_slider->GetValue();

    std::string settings_summary = build_settings_snapshot(*preset_bundle, objectives);

    std::string task_context = "Task: Optimize the full print profile using the provided objectives. "
                               "Favor safe, explainable changes and include advanced settings where relevant.\n"
                               "If the object orientation is suboptimal (e.g., large overhangs, small bed contact), suggest rotation. "
                               "Use keys 'rotation_x', 'rotation_y', 'rotation_z' for relative rotation in degrees (e.g., 90.0, -45.0). "
                               "Only suggest rotation if strictly necessary for printability or strength.";
    ...
    if (canvas) {
        ...
        // Construct params via aggregate initialization
        // struct ThumbnailsParams: sizes, printable_only, parts_only, show_bed, transparent_background, plate_id, use_plate_box
        ...
        if (thumbnail_data.pixels.size() > 0) {
            // Convert to wxImage
            // Determine channels: width * height * channels = size
            int channels = 3;
            if (thumbnail_data.pixels.size() == w * h * 4)
                channels = 4;

            // wxImage expects RGB buffers (malloc'd)
            ...
            if (rgb_data) {
                ...
                wxImage image(w, h, rgb_data, false); // takes ownership
                ...
                image_data_base64 = wxBase64Encode(jpg_data.data(), jpg_data.size()).ToStdString();
                BOOST_LOG_TRIVIAL(info) << "Captured 3D view for AI: " << image_data_base64.size() << " bytes (base64)";
            }
        }
    }

    auto on_success = [this](const std::string& response) {
        // Success Callback (Needs to be on Main Thread)
        // We use CallAfter to marshall back to UI thread
        this->CallAfter([this, response]() {
            m_progress->SetValue(100);
            m_btn_analyze->Enable();

            // Parse the response
            // We expect JSON text from the model, but handle provider wrappers.
            try {
                ...
                try {
                    ...
                    // Keep the original content_text if parsing fails.
                }
...
void AITunerDialog::on_apply(wxCommandEvent& event)
{
    PresetBundle* preset_bundle = wxGetApp().preset_bundle;
    if (!preset_bundle) {
        wxMessageBox(_L("Preset settings are not available yet. Please open a project or restart OrcaSlicer."), _L("AI Settings Tuner"),
    ...
    // Apply the settings from m_analysis_result
    // We expect m_analysis_result to be a JSON string with a 'changes' list.
    try {
        ...
        for (auto& item : pt.get_child("changes")) {
            ...
            if (boost::starts_with(key, "rotation_")) {
                // Handle Rotation
                try {
                    double angle_deg = std::stod(value_norm);
                    ...
                    if (plater) {
                        ...
                        if (!selection.is_empty()) {
                            ...
                            if (axis >= 0) {
                                ...
                                rotation(axis) = angle_rad;

                                // Use World_Relative_Joint for "rotate by X degrees" behavior
                                TransformationType trafo_type = TransformationType::World_Relative_Joint;
                                selection.rotate(rotation, trafo_type);

                                // Need to update the view
                                plater->canvas3D()->do_rotate("AI Rotate");
                                BOOST_LOG_TRIVIAL(info) << "AI Tuner applied rotation: " << key << " = " << angle_deg << " deg";
                                applied_any = true;
                                continue; // Skip the config handling for this key
                            }
                        }
                    }
                } catch (...) {
                    failures.push_back(key + ": invalid angle");
                }
                continue; // It's not a print config setting
            }

            auto in_list = [&key](const std::vector<std::string>& keys) { return std::find(keys.begin(), keys.end(), key) != keys.end(); };

            try {
                ConfigSubstitutionContext substitution_context(ForwardCompatibilitySubstitutionRule::Disable);
                if (in_list(filament_keys)) {
                    new_filament_config.set_deserialize(key, value_norm, substitution_context);
                    changed_filament = true;
                    BOOST_LOG_TRIVIAL(info) << "AI Tuner applied (filament): " << key << " = " << value_norm;
                } else if (in_list(print_keys)) {
                    new_print_config.set_deserialize(key, value_norm, substitution_context);
                    changed_print = true;
                    BOOST_LOG_TRIVIAL(info) << "AI Tuner applied (print): " << key << " = " << value_norm;
                } else if (in_list(printer_keys)) {
                    new_printer_config.set_deserialize(key, value_norm, substitution_context);
                    changed_printer = true;
                    BOOST_LOG_TRIVIAL(info) << "AI Tuner applied (printer): " << key << " = " << value_norm;
                } else if (project_config->has(key)) {
                    new_project_config.set_deserialize(key, value_norm, substitution_context);
                    changed_project = true;
                    BOOST_LOG_TRIVIAL(info) << "AI Tuner applied (project): " << key << " = " << value_norm;
                } else {
                    BOOST_LOG_TRIVIAL(error) << "AI Tuner failed to apply: " << key << " = " << value_norm;
                    failures.push_back(key + ": unsupported setting");
                }
            } catch (const std::exception& e) {
                failures.push_back(key + ": " + e.what());
                BOOST_LOG_TRIVIAL(error) << "AI Tuner failed to apply: " << key << " = " << value_norm << " (" << e.what() << ")";
            }
        }
...
void AITunerDialog::on_dpi_changed(const wxRect& suggested_rect)
{
    // Handle DPI changes
}

void AITunerDialog::on_change_key(wxCommandEvent& event) { prompt_for_api_key(); }

void AITunerDialog::on_provider_changed(wxCommandEvent& event)
{
    update_provider_ui();
    save_ai_settings();
...
void AITunerDialog::on_objective_changed(wxCommandEvent& event)
{
    if (m_quality_value && m_quality_slider)
        m_quality_value->SetLabel(std::to_string(m_quality_slider->GetValue()));
    if (m_speed_value && m_speed_slider)
        m_speed_value->SetLabel(std::to_string(m_speed_slider->GetValue()));
    if (m_strength_value && m_strength_slider)
        m_strength_value->SetLabel(std::to_string(m_strength_slider->GetValue()));

    save_ai_settings();
}

std::string AITunerDialog::selected_provider_key() const
{
    int selection = m_provider_choice ? m_provider_choice->GetSelection() : 0;
    return (selection == 1) ? "local" : "gemini";
}

void AITunerDialog::load_ai_settings()
{
    ...
    AIObjectives objectives = load_ai_objectives(config);
    if (m_quality_slider)
        m_quality_slider->SetValue(objectives.quality);
    if (m_speed_slider)
        m_speed_slider->SetValue(objectives.speed);
    if (m_strength_slider)
        m_strength_slider->SetValue(objectives.strength);
    if (m_quality_value)
        m_quality_value->SetLabel(std::to_string(objectives.quality));
    if (m_speed_value)
        m_speed_value->SetLabel(std::to_string(objectives.speed));
    if (m_strength_value)
        m_strength_value->SetLabel(std::to_string(objectives.strength));
}

void AITunerDialog::save_ai_settings()
{
    AppConfig* config = wxGetApp().app_config;
    if (!config)
        return;
    ...
    if (m_strength_slider)
        objectives.strength = m_strength_slider->GetValue();
    save_ai_objectives(config, objectives);
}

void AITunerDialog::update_provider_ui()
{
...
std::string AITunerDialog::normalize_ai_value(const std::string& key, const std::string& value) const
{
    ...
    if (boost::starts_with(key, "rotation_")) {
        // Just return the number, maybe cleaner?
        return trimmed;
    }
...
void AITunerDialog::update_diff_preview(const std::string& analysis_json)
{
    if (!m_diff_view)
        return;
        ...
        if (auto changes_opt = pt.get_child_optional("changes")) {
            for (auto& item : *changes_opt) {
                ...
                if (in_list(key, filament_keys)) {
                    ...
                    scope_name = "project";
                } else if (boost::starts_with(key, "rotation_")) {
                    scope_name = "geometry";
                }
...
void AITunerDialog::prompt_for_api_key()
{
...
std::string AITunerDialog::load_api_key()
{
...
void AITunerDialog::save_api_key(const std::string& key)
{
    if (AppConfig* config = wxGetApp().app_config) {
