...
static bool is_support_key(const std::string& key)
{
    if (key.find("support") != std::string::npos)
        return true;
    if (key.find("raft") != std::string::npos)
        return true;
    if (key.rfind("brim", 0) == 0)
        return true;
    if (key == "bridge_no_support" || key == "enable_extra_bridge_layer" || key == "max_bridge_length")
        return true;
    return false;
}

AISupportDialog::AISupportDialog(wxWindow* parent)
    : DPIDialog(parent, wxID_ANY, _L("AI Support Strategist"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
    build();
    load_ai_settings();
    update_provider_ui();
...
AISupportDialog::~AISupportDialog() {}

void AISupportDialog::build()
{
    ...
    auto intro_text = new wxStaticText(
        this, wxID_ANY,
        _L("Describe your support needs (e.g., 'clean bottom surfaces', 'easy removal').\n"
           "The AI will tune support-related settings only."));
    ...
    m_provider_choice->Bind(wxEVT_CHOICE, &AISupportDialog::on_provider_changed, this);
    ...
    auto goal_sizer = new wxBoxSizer(wxHORIZONTAL);
    goal_sizer->Add(new wxStaticText(this, wxID_ANY, _L("Support goal:")), 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, 8);
    m_goal_choice = new wxChoice(this, wxID_ANY);
    m_goal_choice->Append(_L("Balanced"));
    m_goal_choice->Append(_L("Easy removal"));
    m_goal_choice->Append(_L("Surface quality"));
    m_goal_choice->Append(_L("Minimal supports"));
    m_goal_choice->Append(_L("Maximum reliability"));
    m_goal_choice->Bind(wxEVT_CHOICE, &AISupportDialog::on_goal_changed, this);
    goal_sizer->Add(m_goal_choice, 1, wxALIGN_CENTER_VERTICAL | wxEXPAND);
    main_sizer->Add(goal_sizer, 0, wxLEFT | wxRIGHT | wxBOTTOM | wxEXPAND, 10);

    m_input_desc = new wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxSize(400, 110), wxTE_MULTILINE);
    main_sizer->Add(m_input_desc, 1, wxALL | wxEXPAND, 10);

    auto btn_sizer = new wxBoxSizer(wxHORIZONTAL);
    m_btn_analyze  = new wxButton(this, wxID_ANY, _L("Analyze Supports"));
    m_btn_analyze->Bind(wxEVT_BUTTON, &AISupportDialog::on_analyze, this);

    m_btn_key = new wxButton(this, wxID_ANY, _L("API Key..."));
    m_btn_key->Bind(wxEVT_BUTTON, &AISupportDialog::on_change_key, this);
    ...
    auto result_label = new wxStaticText(this, wxID_ANY, _L("AI Support Plan:"));
    ...
    m_btn_apply       = new wxButton(this, wxID_ANY, _L("Apply Support Changes"));
    m_btn_apply->Bind(wxEVT_BUTTON, &AISupportDialog::on_apply, this);
    m_btn_apply->Disable();
    ...
    SetMinSize(wxSize(540, 740));
    Fit();
    CenterOnParent();
}

void AISupportDialog::on_analyze(wxCommandEvent& event)
{
    std::string user_input = m_input_desc->GetValue().ToStdString();
    if (user_input.empty()) {
        user_input = "Optimize support settings for the selected goal.";
    }

    PresetBundle* preset_bundle = wxGetApp().preset_bundle;
    if (!preset_bundle) {
        wxMessageBox(_L("Preset settings are not available yet. Please open a project or restart OrcaSlicer."),
                     _L("AI Support Strategist"), wxICON_ERROR | wxOK);
        return;
    }
    ...
    if (provider == "gemini") {
        ...
        if (key.empty()) {
            wxMessageBox(_L("Gemini API key is missing. Please set it first."),
                         _L("AI Support Strategist"), wxICON_WARNING | wxOK);
            prompt_for_api_key();
            return;
        }
        m_gemini_client.set_api_key(key);
    ...
    std::string image_data_base64 = capture_scene_base64();

    std::string task_context =
        "Task: Optimize support-related settings only. "
        "Focus on supports (type, style, thresholds, interface, spacing, Z distances, expansion, brim/raft) and avoid unrelated changes. "
        "Goal: " + support_goal_key() + ".";
...
void AISupportDialog::on_apply(wxCommandEvent& event)
{
    PresetBundle* preset_bundle = wxGetApp().preset_bundle;
    if (!preset_bundle) {
        wxMessageBox(_L("Preset settings are not available yet. Please open a project or restart OrcaSlicer."),
    ...
    try {
        ...
        for (auto& item : pt.get_child("changes")) {
            ...
            if (!is_support_key(key)) {
                failures.push_back(key + ": non-support setting ignored");
                continue;
            }
            ...
            try {
                ...
                if (in_list(key, filament_keys)) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << "AI Support applied (filament): " << key << " = " << value_norm;
                } else if (in_list(key, print_keys)) {
                    new_print_config.set_deserialize(key, value_norm, substitution_context);
                    changed_print = true;
                    BOOST_LOG_TRIVIAL(info) << "AI Support applied (print): " << key << " = " << value_norm;
                    ...
                    BOOST_LOG_TRIVIAL(info) << "AI Support applied (printer): " << key << " = " << value_norm;
                } else if (project_config->has(key)) {
                    new_project_config.set_deserialize(key, value_norm, substitution_context);
                    changed_project = true;
                    BOOST_LOG_TRIVIAL(info) << "AI Support applied (project): " << key << " = " << value_norm;
                } else {
                    BOOST_LOG_TRIVIAL(error) << "AI Support failed to apply: " << key << " = " << value_norm;
                    failures.push_back(key + ": unsupported setting");
                }
            } catch (const std::exception& e) {
                failures.push_back(key + ": " + e.what());
                BOOST_LOG_TRIVIAL(error) << "AI Support failed to apply: " << key << " = " << value_norm << " (" << e.what() << ")";
            }
        }
        ...
        if (applied_any) {
            ...
            wxMessageBox(_L("No valid support setting changes found to apply."), _L("Warning"), wxICON_WARNING | wxOK);
        }
    } catch (const std::exception& e) {
...
void AISupportDialog::on_change_key(wxCommandEvent& event) { prompt_for_api_key(); }

void AISupportDialog::on_provider_changed(wxCommandEvent& event)
{
    update_provider_ui();
    save_ai_settings();
...
void AISupportDialog::on_goal_changed(wxCommandEvent& event)
{
    save_ai_settings();
}

void AISupportDialog::on_dpi_changed(const wxRect& suggested_rect)
{
    // Handle DPI changes
}

std::string AISupportDialog::selected_provider_key() const
{
    int selection = m_provider_choice ? m_provider_choice->GetSelection() : 0;
    return (selection == 1) ? "local" : "gemini";
}

void AISupportDialog::load_ai_settings()
{
    ...
    std::string goal = config ? config->get("ai_support_goal") : "";
    if (m_goal_choice) {
        if (goal == "easy_removal")
            m_goal_choice->SetSelection(1);
        else if (goal == "surface_quality")
            m_goal_choice->SetSelection(2);
        else if (goal == "minimal_supports")
            m_goal_choice->SetSelection(3);
        else if (goal == "max_reliability")
            m_goal_choice->SetSelection(4);
        else
            m_goal_choice->SetSelection(0);
    }
}

void AISupportDialog::save_ai_settings()
{
    AppConfig* config = wxGetApp().app_config;
    if (!config)
        return;
    ...
    config->set("ai_support_goal", support_goal_key());

    config->save();
}

void AISupportDialog::update_provider_ui()
{
...
std::string AISupportDialog::normalize_ai_value(const std::string& key, const std::string& value) const
{
...
void AISupportDialog::update_diff_preview(const std::string& analysis_json)
{
    if (!m_diff_view)
        return;
...
std::string AISupportDialog::capture_scene_base64() const
{
...
std::string AISupportDialog::support_goal_key() const
{
    int selection = m_goal_choice ? m_goal_choice->GetSelection() : 0;
    switch (selection) {
    case 1:
        return "easy_removal";
    case 2:
...
void AISupportDialog::prompt_for_api_key()
{
...
std::string AISupportDialog::load_api_key()
{
...
void AISupportDialog::save_api_key(const std::string& key)
{
    if (AppConfig* config = wxGetApp().app_config) {
